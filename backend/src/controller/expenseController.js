...
export const createExpense = async (req, res) => {
  try {
    ...
    // Validação básica
    if (!usuario_id || !descricao || !valor || !tipo || !recorrencia || !data_inicio) {
    ...
    res.status(201).json({ message: 'Despesa criada com sucesso', despesa: result.rows[0] });
  } catch (err) {
...
export const updateExpense = async (req, res) => {
  try {
    ...
    res.json({ message: 'Despesa atualizada com sucesso', despesa: result.rows[0] });
  } catch (err) {
